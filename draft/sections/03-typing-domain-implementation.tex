...
\paragraph{Meta compilation pipeline}
\label{sec:meta-compilation}
The implementation refines the compilation sketched above into two phases, executed during
grammar loading (\texttt{CompilationPass} in \texttt{rule.rs}):

\textbf{Phase 1 — Fresh meta generation.}
Each occurrence of a user-named meta variable $?A$ is replaced by a fresh
rule-internal meta $_k$ (e.g.\ $_0, _1, \ldots$).
If $?A$ appears in a top-level ascription $b : ?A$ (not nested inside an
arrow), the constraint $\texttt{typeof}(b) = _k$ is added.
For nested occurrences (as in $b : ?A \to ?B$), no $\texttt{typeof}$ constraint
is emitted; the arrow decomposition is deferred to evaluation time.

\textbf{Phase 2 — Shared-meta equality.}
When the same user-named meta $?A$ appears at two distinct positions (e.g.\
in $l : ?A \to ?B$ and $r : ?A$), the two fresh metas $_i, _j$ generated in
phase~1 are linked by an equality constraint $_i = _j$.
After compilation, no user-named metas remain; only fresh internal metas
persist in premises and the conclusion.

\textbf{Evaluation-time decomposition.}
At evaluation time (\texttt{bind\_ascription\_metas} in \texttt{domain.rs}),
the fresh metas embedded in compiled ascriptions are resolved by pattern-matching
the actual type evidence against the structure of the expected type expression.
For $b : _0 \to _1$ with actual evidence $\tau_1 \to \tau_2$, the
meta-substitution map receives $_0 := \tau_1$ (domain) and $_1 := \tau_2$
(codomain).
Equality constraints from phase~2 then propagate these bindings across
premises: $_0 = _2$ forces $\texttt{typeof}(r)$ to match the domain of
$\texttt{typeof}(l)$, implementing the app-rule constraint.

\textbf{Example.}
The STLC application rule
\[
	\frac{\Gamma \vdash l : ?A \to ?B \qquad \Gamma \vdash r : ?A}{?B}\;(\mathit{app})
\]
compiles in three steps:
\begin{enumerate}
	\item $l : ?A \to ?B$ $\to$ fresh $_0, _1$; $l$'s ascription becomes $_0 \to _1$.
	\item $r : ?A$ $\to$ fresh $_2$; shared $?A$ produces $_0 = _2$.
	\item Conclusion $?B$ $\to$ $_1$ (shared $?B$ binds to $_1$).
\end{enumerate}
At evaluation, the meta map $\{_0\!\mapsto\!\mathit{dom}(\tau_l),\; _1\!\mapsto\!\mathit{cod}(\tau_l)\}$
is populated by pattern-matching the ascription $_0 \to _1$ against the actual
type $\tau_l = \texttt{typeof}(l)$.
The equality $_0 = _2$ then checks $\mathit{dom}(\tau_l) = \texttt{typeof}(r)$,
and the conclusion $_1$ yields the codomain.
