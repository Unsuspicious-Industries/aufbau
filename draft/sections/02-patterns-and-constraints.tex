...
\begin{definition}[Type as term]
  ...
  The productions of the designated type fragment $\mathsf{Ty} \in N$ form a
...
\begin{remark}[Subtyping is the subsumption order]
  ...
  The order is used in one direction. A more general (unknown) actual refutes but
  never confirms: a failed unification against a closed expectation forces
  $\mathsf{Lost}$ and stays failed under instantiation, but a success against a
  still-open actual is $\mathsf{Live}$, not $\mathsf{Satisfied}$. Only a ground
  instance, or containment of the actual in the expectation, closes an edge; a
  premature $\mathsf{Satisfied}$ is an unsound accept. So the syntactic layer
  bounds the semantic one without fusing them.
\end{remark}
...
when some edge fails (unifies against $\bot$), and \textsf{Live} otherwise.

\subsection{Context and effects}
\label{sec:effects}

A rule may thread a semantic context $\Gamma \in \mathsf{Ctx}$ and export a
\emph{right-bound effect} $\nabla$ that extends the context seen by later
siblings; a declaration adding a binding is the canonical case. Only an
\emph{exact} node exports an effect, since a prefix node's right boundary is not
yet fixed, and effects compose left to right, applied by $\oplus$
