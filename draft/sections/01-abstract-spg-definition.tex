...
A \emph{constraint domain} is a quintuple
\[
...
\begin{itemize}
  ...
  \item \(\mathsf{Ctx}\) is the set of semantic contexts threaded through the syntactic state, with a distinguished empty context \(\Gamma_0 \in \mathsf{Ctx}\) used to seed parsing and generation;
  ...
  \]
  \item \(\oplus : \mathsf{Ctx} \times \nabla \to \mathsf{Ctx}\) applies a right-bound effect $\nabla$ exported by an exact left sibling to the context of its right sibling. Effects compose left-to-right in the sense of \S\ref{sec:context-flow}.
\end{itemize}
