...
pub fn load_example_grammar(name: &str) -> SPG {
    ...
    SPG::load(&content).unwrap_or_else(|e| panic!("Failed to load grammar '{}': {}", name, e))
}
...
/// "#);
/// ```
pub fn load_inline_grammar(spec: &str) -> SPG {
    SPG::load(spec).expect("Failed to load inline grammar")
}
...
pub mod grammars {
    ...
    static STLC: OnceLock<SPG> = OnceLock::new();
    static CLIKE: OnceLock<SPG> = OnceLock::new();
    static IMP: OnceLock<SPG> = OnceLock::new();
    static FUN: OnceLock<SPG> = OnceLock::new();

    pub fn stlc() -> &'static SPG {
        STLC.get_or_init(|| load_example_grammar("stlc"))
    }

    pub fn clike() -> &'static SPG {
        CLIKE.get_or_init(|| load_example_grammar("clike"))
    }

    pub fn imp() -> &'static SPG {
        IMP.get_or_init(|| load_example_grammar("imp"))
    }

    pub fn fun() -> &'static SPG {
