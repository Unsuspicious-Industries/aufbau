...
fn mk_var_pnt(name: &str) -> PartialNonTerminal {
    PartialNonTerminal {
        production: crate::logic::partial::PartialProduction::from_progress(
            crate::logic::grammar::Production { rule: None, rhs: vec![] },
            0,
            0,
        ),
        children: vec![PartialASTNode::Terminal(PartialTerminal { value: name.to_string(), span: None, binding: None })],
...
fn ascr(name: &str, ty: BoundType) -> BoundTypeAscription {
    BoundTypeAscription { node: mk_var_pnt(name), ty }
}
...
fn conclusion_identity_default() {
    ...
    let ty = tc.apply_bound_rule_partial(&rule, &dummy).expect("rule should apply");
...
fn conclusion_commits_output_extensions() {
    ...
    let dummy = mk_var_pnt("dummy");
    let mut tc = TypeChecker::new();
    let _ = tc.apply_bound_rule_partial(&rule, &dummy).expect("rule should apply");
...
fn conclusion_context_lookup_uses_ambient() {
    ...
    let dummy = mk_var_pnt("dummy");
    let mut tc = TypeChecker::new();
    tc.context_mut().add("z".into(), BoundType::Atom("T".into()));
    let ty = tc.apply_bound_rule_partial(&rule, &dummy).expect("rule should apply");
