...
impl Symbol {
    ...
    pub fn group_symbols(&self) -> Option<&[Symbol]> {
        match self {
            Symbol::Group { symbols, .. } => Some(symbols.as_slice()),
            _ => None,
        }
    }
    pub fn group_symbols_mut(&mut self) -> Option<&mut Vec<Symbol>> {
        match self {
            Symbol::Group { symbols, .. } => Some(symbols),
            _ => None,
        }
    }
    pub fn is_group(&self) -> bool {
        matches!(self, Symbol::Group { .. })
    }
...
pub(crate) mod tests {
    ...
    fn parses_unified_stlc_grammar() {
        ...
        let var_symbol = typed_param_prod[0]
            .rhs
            .iter()
            .find(|s| s.value() == "Variable")
            .unwrap();
        ...
        match &var_rule.premises[0] {
            typing::Premise {
                setting: None,
                judgment: Some(typing::TypingJudgment::Membership(var, ctx)),
            } => {
                assert_eq!(var, "x");
                assert_eq!(ctx, "Γ");
            }
        ...
        match &lambda_rule.premises[0] {
            typing::Premise {
                setting,
                judgment: Some(typing::TypingJudgment::Ascription((term, ty))),
            } => {
        ...
        assert_eq!(
            grammar.start_nonterminal().cloned(),
            Some("Term".to_string())
        );
