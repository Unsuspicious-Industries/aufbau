...
/// A single transition in a grammar path. The parser traverses to `i`
/// and optionally constrains the child non-terminal to `a`.
...
/// A concrete grammar-space path represented as a finite sequence of
/// `PathStep`s. Unlike `TreePath`, which is just a list of child indices in a
/// realized tree, `GrammarPath` also records the production alternative chosen
/// at each step.
