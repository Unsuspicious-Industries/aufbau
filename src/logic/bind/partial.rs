...
pub trait BindingResolver {
    fn resolve_rule(&self, rule: &TypingRule, node: &PartialNonTerminal) -> Result<BoundTypingRule, String>;
    fn resolve_premise(&self, premise: &Premise, node: &PartialNonTerminal) -> Result<BoundPremise, String>;
    fn resolve_conclusion(&self, conclusion: &Conclusion, node: &PartialNonTerminal) -> Result<BoundConclusion, String>;
}
...
impl BindingResolver for DefaultBindingResolver {
    fn resolve_rule(&self, rule: &TypingRule, node: &PartialNonTerminal) -> Result<BoundTypingRule, String> {
        ...
        Ok(BoundTypingRule { name: rule.name.clone(), premises: bound_premises, conclusion: bound_conclusion })
    }
    ...
    fn resolve_premise(&self, premise: &Premise, node: &PartialNonTerminal) -> Result<BoundPremise, String> {
        ...
        let bound_judgment = match &premise.judgment {
            Some(TypingJudgment::Ascription((term, ty))) => {
                let var_node = get_nt_binding_partial(node, term.clone())
                    .ok_or_else(|| format!("Could not resolve binding variable {} in ascription", term))?;
                let resolved_type = bind_type_partial(node, ty.clone())
                    .ok_or_else(|| format!("Could not resolve type binding for type {:?} in ascription", ty))?;
                BoundTypingJudgment::Ascription(BoundTypeAscription { node: var_node, ty: resolved_type })
            }
            Some(TypingJudgment::Membership(var, ctx)) => {
                let var_node = get_nt_binding_partial(node, var.clone())
            ...
            None => { return Ok(BoundPremise { setting: bound_setting, judgment: None }); }
        };

        Ok(BoundPremise { setting: bound_setting, judgment: Some(bound_judgment) })
    }
...
impl DefaultBindingResolver {
    fn resolve_type_setting(&self, setting: &TypeSetting, node: &PartialNonTerminal) -> Result<BoundTypeSetting, String> {
        ...
        for (term, ty) in &setting.extensions {
            ...
            bound_extensions.push(BoundTypeAscription { node: var_node, ty: resolved_type });
        }
        Ok(BoundTypeSetting { name: setting.name.clone(), extensions: bound_extensions })
    }

    fn resolve_premise_many(&self, premise: &Premise, node: &PartialNonTerminal) -> Result<Vec<BoundPremise>, String> {
        let bound_setting = if let Some(setting) = &premise.setting {
            Some(self.resolve_type_setting(setting, node)?)
        } else { None };
...
fn direct_nonterminal_children(root: &PartialNonTerminal) -> Vec<PartialNonTerminal> {
    ...
    for ch in &root.children {
        if let PartialASTNode::NonTerminal(alts) = ch {
            for alt in alts { out.push(alt.clone()); }
        }
    }
    out
}

fn find_parent_with_binding_level_partial(root: &PartialNonTerminal, var: &str) -> Option<PartialNonTerminal> {
    ...
    for ch in direct {
        if let Some(p) = find_parent_with_binding_level_partial(&ch, var) { return Some(p); }
    }
    None
}

pub fn collect_nt_bindings_same_level_partial(root: &PartialNonTerminal, var: &str) -> Vec<PartialNonTerminal> {
...
pub fn extract_terminal_value_partial(node: &PartialASTNode) -> Option<String> {
    ...
    match filtered.len() { 1 => Some(filtered[0].clone()), _ => None }
}
...
pub fn get_nt_binding_partial(node: &PartialNonTerminal, var: String) -> Option<PartialNonTerminal> {
    if let Some(b) = &node.binding { if *b == var { return Some(node.clone()); } }
    for ch in direct_nonterminal_children(node) {
        if let Some(found) = get_nt_binding_partial(&ch, var.clone()) { return Some(found); }
    }
    None
}

pub fn get_var_binding_partial(node: &PartialNonTerminal, var: String) -> Result<Option<String>, String> {
...
pub fn bind_type_partial(node: &PartialNonTerminal, type_var: crate::logic::typing::Type) -> Option<BoundType> {
    ...
    match type_var {
        Type::Atom(var) => {
            ...
            if let Some(nt) = get_nt_binding_partial(node, var.clone()) {
                if let Some(full_ty) = get_type_value_partial(&nt) { return Some(full_ty); } else { return None; }
            }
            None
        }
        ...
        Type::Tuple(v) => {
            ...
            if elements.is_empty() { return Some(BoundType::Tuple(Vec::new())); }
            let tuple_elem_types: Vec<BoundType> = elements.into_iter().filter_map(|nt| get_type_value_partial(&nt)).collect();
        ...
        Type::Array(t, size) => {
            ...
            if size_str.is_err() { return None; }
            let size_u64 = if let Some(s) = size_str.unwrap() { s.parse::<u64>().ok() } else { None };
            match size_u64 { Some(n) => Some(BoundType::Array(Box::new(b), n)), None => None }
        }
...
pub trait BindablePartialNonTerminal {
    fn resolve_typing_rule_with<R: BindingResolver>(&self, resolver: &R, rule: &TypingRule) -> Result<BoundTypingRule, String>;
}
...
pub fn branch_matches_expected_type(node: &PartialNonTerminal, rule: &TypingRule, expected: &crate::logic::typing::Type) -> bool {
...
pub fn select_branches_by_type<'a>(alts: &'a [PartialNonTerminal], rules: &[TypingRule], expected: &crate::logic::typing::Type) -> Vec<&'a PartialNonTerminal> {
    let mut out = Vec::new();
    for alt in alts {
        let mut ok = false;
        for rule in rules {
