...
mod tests {
    use crate::logic::{partial::PartialParser, typing::{TypingRule, Type}};
    use crate::logic::bind::{BoundConclusionKind};
    use crate::logic::bind::partial::{conclude_type_with_rule, select_branches_by_type, DefaultBindingResolver, BindingResolver};
    ...
    fn stlc_type_guided_branch_selection_and_typecheck() {
        ...
        let mut g = crate::logic::grammar::Grammar::load(crate::logic::grammar::tests::STLC_SPEC).unwrap();
        // Ensure start symbol is set
        if g.start_nonterminal().is_none() {
            if let Some(first) = g.productions.keys().next().cloned() { g.set_start(first); }
        }
        let mut p = PartialParser::new(g.clone());

        // Build rules equivalent to STLC ones
        let var_rule = TypingRule::new(
            "x ∈ Γ".to_string(),
            "Γ(x)".to_string(),
            "var".to_string()
        ).unwrap();
        ...
        tc.context_mut().add("z".to_string(), crate::logic::bind::typing::BoundType::Atom("a".to_string()));
        ...
        for r in [&lambda_rule, &app_rule, &var_rule] {
            if let Ok(br) = resolver.resolve_rule(r, &branch) {
                if let BoundConclusionKind::Type(_) = br.conclusion.kind { concluded = true; break; }
            }
        }
        assert!(concluded);
    }
