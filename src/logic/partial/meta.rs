...
impl MetaParser {
    ...
    pub fn parse(&mut self, input: &str) -> Result<(PartialAST, usize), String> {
    ...
    pub fn partial(&mut self, input: &str) -> Result<(PartialAST, usize), String> {
        let mut current_depth = self.start_depth;
...
mod tests {
    ...
    fn test_meta_parser_simple() {
        ...
        let (ast, depth) = mp.parse("n + n + n").unwrap();
    ...
    fn test_meta_parser_deep_nesting() {
        ...
        let (ast, depth) = mp.parse("f ( f ( x ) )").unwrap();
        assert!(ast.is_complete());
    ...
    fn test_meta_parser_highly_ambiguous() {
        ...
        let (ast, depth) = mp.parse("x + x + x").unwrap();
        assert!(ast.is_complete());
    ...
    fn test_meta_parser_incremental_cache() {
        ...
        let (ast, depth) = mp.parse("( ( ( x ) ) )").unwrap();
        assert!(ast.is_complete());
