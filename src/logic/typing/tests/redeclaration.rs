...
fn test_context_extend_rejects_duplicate() {
    ...
    let ctx = ctx.extend("x".to_string(), Type::Raw("Int".to_string()))
    ...
    assert!(result.is_err(), "Context should reject duplicate variable 'x'");
    assert!(result.unwrap_err().contains("already contains binding for 'x'"),
...
fn test_context_extend_allows_different_variables() {
    ...
    let ctx = ctx.extend("y".to_string(), Type::Raw("Bool".to_string()))
        .expect("Second binding with different name should succeed");
    
    let ctx = ctx.extend("z".to_string(), Type::Raw("String".to_string()))
...
fn test_nested_lambda_different_variables_accepted() {
    ...
    let completes = ast.completes();
    assert!(!completes.is_empty(), "Should have at least one complete parse");
...
fn test_context_multiple_duplicates() {
    ...
    assert!(ctx.extend("x".to_string(), Type::Raw("Bool".to_string())).is_err());
    assert!(ctx.extend("x".to_string(), Type::Raw("String".to_string())).is_err());
    assert!(ctx.extend("x".to_string(), Type::Atom("A".to_string())).is_err());
...
fn test_sequential_scopes_same_variable_rejected() {
    ...
    let ctx1 = ctx.extend("x".to_string(), Type::Raw("Int".to_string()))
        .expect("First x should work");
    
    // Add y in second "scope" (based on ctx1)
    let ctx2 = ctx1.extend("y".to_string(), Type::Raw("Bool".to_string()))
    ...
    assert!(result.is_err(), "Should not be able to rebind x in cumulative context");
}
