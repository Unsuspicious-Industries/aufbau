...
use crate::engine::rank::{DefaultRanker, Ranker, StlcRanker};
...
fn synthesizes_complete_with_repetition() {
    ...
    let program = synth.run("", 10).expect("synthesis");
    ...
    let past = parser.partial(&program).expect("partial parse of synthesized program");
    assert!(past.is_complete(), "synthesized program should be complete");
}
...
fn synthesize_xtlc() {
    ...
    let program = synth.synthesize("λx:(A", 1000, 500, 10).expect("synthesis");
    assert!(!program.is_empty());
