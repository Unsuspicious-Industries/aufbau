...
fn rejecting_typing_returns_err() {
    // finalize rejects, so no node — not even partial — is created for the
    // start symbol at any span, and parse() returns Err.
    let grammar = SPG::load(REJECT_GRAMMAR).unwrap();
    let mut parser = stub_parser(grammar);
    assert!(
...
fn parse_regex_terminal_matches_pattern() {
    let grammar = SPG::load("Start ::= /[0-9]+/").unwrap();
    let mut parser = stub_parser(grammar);
...
fn parse_returns_correct_text() {
    let grammar = SPG::load("Start ::= 'hello'").unwrap();
    let mut parser = stub_parser(grammar);
