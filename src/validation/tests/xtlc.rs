...
pub (crate) fn xtlc_grammar() -> Grammar {
...
fn perf_profile_lambda_dot_case() {
    ...
    let (result, _) = timed_complete(&grammar, input, 2, None, None);
    let cold_time = start.elapsed();
    println!("Cold cache time: {:?}", cold_time);
    
    // Warm run (same input)
    let start = Instant::now();
    let (_, _) = timed_complete(&grammar, input, 2, None, None);
    let warm_time = start.elapsed();
    println!("Warm cache time: {:?}", warm_time);
    
    match result {
        CompletionResult::Success { complete_input, depth, .. } => {
            println!("Success: '{}' (depth {})", complete_input, depth);
        }
        other => println!("Result: {:?}", other),
    }
    
    if cold_time > warm_time {
        println!("Cache speedup: {:.1}x", cold_time.as_secs_f64() / warm_time.as_secs_f64());
    }
}
...
fn perf_profile_cache_benefit() {
    ...
    if cold_time > warm_time {
        println!("✓ Cache provides {:.1}x speedup", cold_time.as_secs_f64() / warm_time.as_secs_f64());
    }
}
