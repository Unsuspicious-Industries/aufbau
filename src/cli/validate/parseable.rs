...
use p7::validation::parseable::{self, run_parse_batch, ParseTestCase};
use rayon::prelude::*;
...
pub fn run(args: &ValidateCmd) {
    eprintln!("p7 validation runner - parseable");
    ...
    for (name, grammar, valids, invalids) in modules {
        ...
        eprintln!("Running parseable '{}' ({} valid + {} invalid)", name, valids.len(), invalids.len());
        ...
        let case_pb = mp.insert_after(&suite_pb, indicatif::ProgressBar::new(all_cases.len() as u64));
        ...
        let indexed_cases: Vec<(usize, ParseTestCase)> = all_cases.into_iter().enumerate().collect();

        // Build worker pool if requested
        let thread_pool = match args.jobs {
            Some(n) if n > 0 => Some(ThreadPoolBuilder::new().num_threads(n).build().expect("failed to create thread pool")),
            _ => None,
        };

        let mut par_results: Vec<(usize, serde_json::Value, parseable::ParseResult, std::time::Duration, u128)> = if let Some(pool) = &thread_pool {
            let mut v = Vec::new();
            pool.install(|| {
                v = indexed_cases
                    .par_iter()
                    .map(|(idx, case)| {
                        ...
                        let (passed_flag, prefix_count, failing_prefix, error, prefix_index) = match &result {
                            parseable::ParseResult::Pass { prefix_count, .. } => (true, Some(*prefix_count as usize), None::<String>, None::<String>, None::<usize>),
                            parseable::ParseResult::Fail { failing_prefix, error, prefix_index } => (false, None, Some(failing_prefix.clone()), Some(error.clone()), Some(*prefix_index)),
                        };
        ...
        for (_idx, case_obj, result, duration, _start_ts) in par_results {
            ...
            match &result {
                parseable::ParseResult::Pass { .. } => {
                    println!("✓ ({:?}) {} '{}'", duration, case_obj.get("desc").and_then(|v| v.as_str()).unwrap_or(""), input);
                }
                parseable::ParseResult::Fail { failing_prefix, error, .. } => {
                    println!("✗ {} '{}'", case_obj.get("desc").and_then(|v| v.as_str()).unwrap_or(""), input);
            ...
            let expect = if case_obj.get("xfail").and_then(|v| v.as_bool()).unwrap_or(false) { "FAIL" } else { "PASS" }.to_string();
            let detail = match result {
                parseable::ParseResult::Pass { .. } => String::new(),
                parseable::ParseResult::Fail { failing_prefix, error, .. } => format!("Failing prefix: '{}'\nError: {}\n", failing_prefix, error),
            };

            results.push(CaseResult {
                suite: name,
                desc: case_obj.get("desc").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                input: input.to_string(),
                expect,
                passed,
                duration,
                detail,
                prefix_count: case_obj.get("prefix_count").and_then(|v| v.as_u64()).map(|n| n as usize),
                prefix_index: case_obj.get("prefix_index").and_then(|v| v.as_u64()).map(|n| n as usize),
            });
    ...
    if let Some(profile_path) = &args.profile {
        ...
        let perf_obj = json!({
            "generated_by": "p7 validate parseable",
            "variant": "performance",
    ...
    writeln!(f, "  cases={} passed={} failed={}", total_cases, passed, failed).ok();
    writeln!(f, "===============================================================================").ok();
    writeln!(f).ok();
    ...
    if failed == 0 {
        ...
        for r in &results {
            if !r.passed {
                ...
                writeln!(f).ok();
                writeln!(f, "    FAIL  {}  (expect={}, time={}us)", r.desc, r.expect, r.duration.as_micros()).ok();
    ...
    for suite_name in unique_suites {
        let suite_results: Vec<&CaseResult> = results.iter().filter(|r| r.suite == suite_name).collect();
        ...
        writeln!(f, "    {:4}  {:40} passed={} failed={} time={}us", status, suite_name, sp, sf, st.as_micros()).ok();
    }
    writeln!(f).ok();
    ...
    writeln!(sf, "  cases={} passed={} failed={}", total_cases, passed, failed).ok();
    ...
    for r in &results {
        if r.passed {
            ...
            writeln!(sf, "    ok  {:30} {:>8}us  \"{}\"", r.desc, r.duration.as_micros(), r.input).ok();
        }
    }
    ...
    writeln!(ff, "  cases={} passed={} failed={}", total_cases, passed, failed).ok();
    writeln!(ff, "==============================================================================").ok();
    writeln!(ff).ok();
    ...
    if failed == 0 {
        ...
        for r in &results {
            if !r.passed {
                ...
                writeln!(ff).ok();
                writeln!(ff, "    FAIL  {}  (expect={}, time={}us)", r.desc, r.expect, r.duration.as_micros()).ok();
    ...
    if failed == 0 {
        eprintln!(
            "  ALL {} CASES PASSED  beam_fallbacks=?  ",
            total_cases,
        );
    } else {
        eprintln!(
