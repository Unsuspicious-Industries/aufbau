...
#[test]
fn capture_transparent_for_matching() {
...
#[test]
fn simplify_preserves_groups() {
    let r = Regex::cat(Regex::Group(0, Box::new(Regex::Char('a'))), Regex::Epsilon);
    let s = r.simplify();
    assert!(matches!(s, Regex::Group(0, _)));
}

#[test]
fn to_pattern_strips_groups() {
    let r = Regex::new("(a)(b)").unwrap();
    let p = r.to_pattern();
    let r2 = Regex::new(&p).unwrap();
    assert!(r2.matches("ab"));
}
