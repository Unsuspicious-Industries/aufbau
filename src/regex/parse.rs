...
impl Regex {
    ...
    pub fn from_hir(hir: &regex_syntax::hir::Hir) -> Self {
        match hir.kind() {
            ...
            HirKind::Class(Class::Bytes(c)) => c
                .iter()
                .map(|r| {
                    if r.start() == r.end() {
                        Self::Char(r.start() as char)
                    } else {
                        Self::Range(r.start() as char, r.end() as char)
                    }
                })
                .reduce(Self::or)
                .unwrap_or(Self::Empty),
            ...
            HirKind::Repetition(rep) => {
                ...
                match (rep.min, rep.max) {
                    ...
                    (n, None) => {
                        Self::cat(Self::repeat(inner.clone(), n as usize), Self::star(inner))
                    }
                    (n, Some(m)) if n == m => Self::repeat(inner, n as usize),
                    (n, Some(m)) => Self::cat(
                        Self::repeat(inner.clone(), n as usize),
                        Self::repeat_opt(inner, (m - n) as usize),
                    ),
                }
            }
    ...
    fn repeat(r: Self, n: usize) -> Self {
        (0..n).fold(Self::Epsilon, |acc, _| Self::cat(acc, r.clone()))
    }
    fn repeat_opt(r: Self, n: usize) -> Self {
        (0..n).fold(Self::Epsilon, |acc, _| Self::cat(acc, Self::opt(r.clone())))
    }
}
