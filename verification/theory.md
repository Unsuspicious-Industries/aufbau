...
- we want to run them from the verification entrypoint (`check.sh`)
...
The `verification/check.sh -f ...` pipeline uses the Rocq layer as
an external oracle for Rust completions.
